<div class="form_control {{$col}}">
    ...
    <div class="input-container">
        <input id="{{$id}}" name="{{$name}}" value="{{old($name,$value)}}" type="number"
               class="form-control myInput MobilePhone @error($name) is-invalid @enderror "
               data-parsley-pattern="\d+"
        >
        @error($name)
        <div class="invalid_feedback_Div" role="alert">{{ \App\Helpers\AdminHelper::error($message,$name,$label) }}</div>
        @enderror
    </div>
</div>
...
@section('StyleFile')
    {!! $MinifyTools->setWebAssets('assets/portal/')->MinifyCss('intlTelInput/css/intlTelInput.css',$cssMinifyType,true) !!}
    {!! $MinifyTools->setWebAssets('assets/portal/')->MinifyCss('intlTelInput/css/custom.css',$cssMinifyType,true) !!}

@endsection


@section('AddScript')
    <script src="{{ defPortalAssets('intlTelInput/js/intlTelInput_'.thisCurrentLocale().'.js') }}"></script>
@endsection
