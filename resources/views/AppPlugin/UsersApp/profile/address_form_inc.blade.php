<div class="card profile_card">
    ...
    <div class="card-body">
        ...
        <form action="{{route('UsersApp_ProfileAddressSave')}}" method="post" class="myForm mb__10">
            @csrf
            <input type="hidden" name="page_type" value="{{$pageType}}">

            <div class="form-row">
                <x-site.form.input name="recipient_name" label="{!! __('web/profile.form_recipient_name') !!}" :value="old('recipient_name',$UserProfile->name)" col="8"/>
            </div>

            <div class="form-row">
                <x-site.form.phone name="phone" :label="__('web/profile.form_mobile')" col="6" :value="old('phone',$UserProfile->phone)"/>
                <x-site.form.phone name="phone_option" :label="__('web/profile.form_phone_option')" col="6" :value="old('phone_option')"/>
             </div>
